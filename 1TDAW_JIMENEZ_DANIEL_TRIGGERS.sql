...
INSERT INTO notas VALUES ('44444444D', 100, 8.5);
INSERT INTO notas VALUES ('22222222B', 100, 9.0);
INSERT INTO notas VALUES ('99999999Z', 100, 7.0);

//Ejercicio 4//

CREATE TABLE alumAfectado(
    dni VARCHAR(10) PRIMARY KEY
);
...
FOR EACH ROW
DECLARE
    nueva_media NUMBER(4,2);
    dni_afectado VARCHAR2(10);
BEGIN
IF DELETING THEN
    dni_afectado := :old.dni;
ELSE
    dni_afectado := :new.dni;
END IF;
    SELECT AVG(nota) INTO nueva_media 
FROM notas 
WHERE dni = dni_afectado;
    
UPDATE alumno 
SET nota_media = nueva_media 
WHERE dni = dni_afectado;
    DBMS_OUTPUT.PUT_LINE('Nota media actualizada para el alumno ' || dni_afectado || ',' || nueva_media);
EXCEPTION
WHEN NO_DATA_FOUND THEN
    UPDATE alumno 
    SET nota_media = NULL 
    WHERE dni = dni_afectado;
    DBMS_OUTPUT.PUT_LINE('Nota media NULL para el alumno ' || dni_afectado);
END actuaNotaMedia;
